...
class LocalVectorStore(VectorStore):
    ...
    async def upsert(self,chunks): self.chunks.extend(chunks)
    async def search(self,query,filters=None,limit=10):
        from app.rag.retrieval import hybrid_retrieve
        return hybrid_retrieve(query,self.chunks,limit)
    async def delete_repository(self,repository): self.chunks=[]
    async def health(self): return {"status":"ok","backend":"local","chunks":len(self.chunks)}
