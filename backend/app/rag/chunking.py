        ...
        try:
            ...
            for node in tree.body:
                if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)):
                    ...
                    out.append(
                        Chunk(_id(rel, start, end, body), rel, start, end, node.name, lang, body)
                    )
        except SyntaxError:
            pass
    if not out:
        ...
        for i in range(0, len(lines), step):
            ...
            if body.strip():
                out.append(
                    Chunk(
                        _id(rel, i + 1, min(i + step, len(lines)), body),
                        rel,
                        i + 1,
                        min(i + step, len(lines)),
                        None,
                        lang,
                        body,
                        "documentation" if path.suffix in {".md", ".txt"} else "code",
                    )
                )
    return out
