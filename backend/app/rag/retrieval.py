...
def tokens(s:str): return re.findall(r"[A-Za-z_][A-Za-z0-9_]*|\d+", s.lower())
...
def hybrid_retrieve(query:str, chunks:list[Chunk], top_k:int=8):
    lex=sorted(chunks,key=lambda c:lexical_score(query,c.text),reverse=True)
    dense=sorted(chunks,key=lambda c:dense_score(query,c.text),reverse=True)
    fused=reciprocal_rank_fusion([[c.id for c in lex[:30]],[c.id for c in dense[:30]]])
    by_id={c.id:c for c in chunks}
    return [(by_id[i],s) for i,s in sorted(fused.items(),key=lambda x:x[1],reverse=True)[:top_k]]

class Reranker:
    def rerank(self, query, items):
        # Configurable deterministic reranker; replace with cross-encoder in production.
        return sorted(items,key=lambda x:(lexical_score(query,x[0].text)*0.7+dense_score(query,x[0].text)*0.3),reverse=True)
