...
@router.get("/health")
async def health():
    return {"status": "ok", "service": "enterprise-ai-incident-agent"}
...
async def repository_status(repo_id: str):
    return repository_service.status(repo_id)
...
async def stream(iid: str, api_key: str):
    ...
    async def gen():
        ...
        while True:
            ...
            if status in {"SUCCESS", "FAILED", "WAITING_APPROVAL", "REJECTED"}:
                break
...
async def approve(iid: str, body: ApprovalInput):
    return await investigation_service.approve(iid, body.comment)


@router.post("/api/v1/investigations/{iid}/reject", dependencies=[Depends(require_api_key)])
async def reject(iid: str, body: ApprovalInput):
    return await investigation_service.reject(iid, body.comment)


@router.post(
    "/api/v1/investigations/{iid}/request-changes", dependencies=[Depends(require_api_key)]
)
async def request_changes(iid: str, body: RequestChangesInput):
    return await investigation_service.request_changes(iid, body.comment)
