...
class GitHubAdapter:
    ...
    async def create_pr(self, investigation_id: str, patch: str):
        ...
        if not s.github_write_enabled:
            return {
                "mode": "simulation",
                "branch": f"ai-fix/{investigation_id[:8]}",
                "status": "SIMULATED",
                "message": "GitHub writes disabled; no network mutation performed.",
            }
        if not s.github_token:
            raise RuntimeError("GITHUB_WRITE_ENABLED=true but GITHUB_TOKEN is missing")
        raise RuntimeError(
            "Real GitHub network adapter requires organization/repository coordinates; simulation is the safe default."
        )
